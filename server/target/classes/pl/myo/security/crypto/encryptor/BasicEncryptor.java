package pl.myo.security.crypto.encryptor;

import org.springframework.stereotype.Component;
import pl.myo.security.crypto.ICryptoConstants;
import pl.myo.security.crypto.exception.EncryptorException;
import pl.myo.security.crypto.key.KeyService;
...
public class BasicEncryptor
{
   ...
   public byte[] encrypt(byte[] input) throws EncryptorException
   {
      try
      {
         Cipher cipher = Cipher.getInstance(ICryptoConstants.PADDED_ENCRYPTION_ALGORITHM);
   ...
   protected byte[] getMac(byte[] input, Key tokenKey) throws EncryptorException
   {
      try
      {
         Mac mac = Mac.getInstance(ICryptoConstants.MAC_ALGORITHM);
         mac.init(new SecretKeySpec(tokenKey.getEncoded(), ICryptoConstants.KEY_ALGORITHM));
